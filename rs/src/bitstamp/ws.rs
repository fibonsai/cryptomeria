...
use crate::bitstamp::types::{
    BitstampWsMessage, MessageType, OrderBookData, TradeData, display_message,
};
...
impl ExchangeClientBuilder for BitstampClient {
    fn with_sender(self, sender: Sender) -> Self {
        BitstampClient::with_sender(self, sender)
    }
    fn with_retention_window(self, hours: u64) -> Self {
        BitstampClient::with_retention_window(self, hours)
    }
    fn with_metrics_port(self, port: u16) -> Self {
        BitstampClient::with_metrics_port(self, port)
    }
    fn with_data_output(self, enabled: bool) -> Self {
        BitstampClient::with_data_output(self, enabled)
    }
    fn with_cli_instrument(self, inst_id: String) -> Self {
        BitstampClient::with_cli_instrument(self, inst_id)
    }
    fn with_lob_metrics(self, metrics: Arc<LobMetrics>) -> Self {
        BitstampClient::with_lob_metrics(self, metrics)
    }
            ...
            MessageType::L2Snapshot | MessageType::L2Update => {
                ...
                if !levels.is_empty() {
                    let okx_levels: Vec<(String, crate::okx::types::LobLevel)> = levels
                        ...
                        .collect();
                    persist_lob(sender, cli_inst_id, exchange, ts_ms, "update", &okx_levels)
                        .await?;
                }
            }
            MessageType::Trade => {
